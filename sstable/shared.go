...
func (i *tableIterator) SetUpperBoundInclusive(upperBoundInclusive bool) {
	type upperBoundInclusiveSetter interface {
		SetUpperBoundInclusive(upperBoundInclusive bool)
	}
	i.Iterator.(upperBoundInclusiveSetter).SetUpperBoundInclusive(upperBoundInclusive)
}
...
func (i *tableIterator) isKeyDeleted(k *InternalKey) bool {
	...
	if i.rangeDelIter != nil {
		cmp := i.getCmp()
		span := i.rangeDelIter.SeekGE(k.UserKey)
...
func (i *tableIterator) seekLTShared(key []byte, flags base.SeekLTFlags) (*InternalKey, base.LazyValue) {
	r, cmp := i.getReader(), i.getCmp()
	ib := i.cmpSharedBound(key)
	if ib < 0 {
		i.setExhaustedBounds(-1)
...
func (i *tableIterator) nextShared() (*InternalKey, base.LazyValue) {
	cmp := i.getCmp()
...
func (i *tableIterator) prevShared() (*InternalKey, base.LazyValue) {
	cmp := i.getCmp()
